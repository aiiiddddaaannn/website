...
export default function ExperienceCard({ url, discordUrl, vouchesUrl, groupUrl, title, fullDescription, cardDescription, cardImage, media, delay, gradient, myRole, timeline }: { url?: string, discordUrl?: string, vouchesUrl?: string, groupUrl?: string, title: string, fullDescription: string[], cardDescription: string, cardImage: string, media: string[], delay: number, gradient: string, myRole: string, timeline: string }) {
                ...
                <div className={`p-4 flex md:flex-row flex-col gap-6 ${gradient} from-primary to-secondary rounded-lg border-1 border-accent shadow-2xl shadow-background items-center`}>
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex row gap-4 mt-2">
                            {discordUrl &&
                                <LinkButton label="Visit Discord" link={discordUrl} width={(vouchesUrl && url) || (vouchesUrl && groupUrl) || (url && groupUrl) ? "w-1/3" : (vouchesUrl || url || groupUrl) ? "w-1/2" : "w-full"} />
                            }
                            {vouchesUrl &&
                                <LinkButton label="View Vouches" link={vouchesUrl} width={(discordUrl && url) || (discordUrl && groupUrl) || (url && groupUrl) ? "w-1/3" : (discordUrl || url || groupUrl) ? "w-1/2" : "w-full"} />
                            }
                            {groupUrl &&
                                <LinkButton label="Visit Group" link={groupUrl} width={(discordUrl && vouchesUrl) || (discordUrl && url) || (vouchesUrl && url) ? "w-1/3" : (discordUrl || vouchesUrl || url) ? "w-1/2" : "w-full"} />
                            }
                            {url &&
                                <LinkButton label="Visit Website" link={url} width={(discordUrl && vouchesUrl) || (discordUrl && groupUrl) || (vouchesUrl && groupUrl) ? "w-1/3" : (discordUrl || vouchesUrl || groupUrl) ? "w-1/2" : "w-full"} />
                            }
                        </div>
                    </div>
