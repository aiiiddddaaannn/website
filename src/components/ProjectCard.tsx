...
export default function ProjectCard({ url, discordUrl, title, fullDescription, cardDescription, cardImage, media, delay, gradient, myRole }: { url?: string, discordUrl?: string, title: string, fullDescription: string[], cardDescription: string, cardImage: string, media: string[], delay: number, gradient: string, myRole: string }) {
    ...
    <>
      ...
      >
        <div className={`p-4 flex flex-col ${gradient} from-primary to-secondary rounded-lg border-1 border-accent shadow-2xl shadow-background items-center`}>
          <img alt="" draggable={false} className="rounded-lg border-1 border-accent mb-4 duration-300 md:h-[14rem] sm:h-[12rem] h-[10rem] w-auto object-contain" src={cardImage} />
          ...
          <div className="flex flex-row gap-2 w-full mt-3">
            {discordUrl &&
              <LinkButton label="Visit Discord" link={discordUrl} width={url ? "w-1/2" : "w-full"} />
            }
            {url &&
              <LinkButton label="Visit Website" link={url} width={discordUrl ? "w-1/2" : "w-full"} />
            }
          </div>
        </div>
